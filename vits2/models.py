...
from vits2 import monotonic_align
from vits2.commons import get_padding
from vits2.commons import init_weights
from vits2.pqmf import PQMF
from vits2.qwfp import TextEncoderWithQWFP
from vits2.stft import OnnxSTFT
from vits2.stft import TorchSTFT
        ...
        if use_transformer_flows:
            if transformer_flow_type == 'pre_conv':
                ...
                for i in range(n_flows):
                    ...
                    self.flows.append(modules.Flip())
                    self.flows.append(
                        MonoTransformerFlowLayer(channels, hidden_channels, mean_only=True)
                    )
            ...
            for i in range(n_flows):
                ...
                self.flows.append(
                    MonoTransformerFlowLayer(
                        channels, hidden_channels, mean_only=True, residual_connection=True
                    )
                )
        else:
            for i in range(n_flows):
...
class Multiband_iSTFT_Generator(torch.nn.Module):  # !
    ...
    def forward(self, x, g=None):
        ...
        y_mb_hat = stft.inverse(
            ...
            torch.reshape(
                phase,
                (phase.shape[0] * self.subbands, self.gen_istft_n_fft // 2 + 1, phase.shape[-1]),
            ),
        )
...
class Multistream_iSTFT_Generator(torch.nn.Module):
    ...
    def forward(self, x, g=None):
        ...
        y_mb_hat = F.conv_transpose1d(
            y_mb_hat, self.updown_filter.to(x.device) * self.subbands, stride=self.subbands
        )
...
class DiscriminatorP(torch.nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, use_spectral_norm=False):
        ...
        self.convs = nn.ModuleList([
            ...
            norm_f(
                Conv2d(1024, 1024, (kernel_size, 1), 1, padding=(get_padding(kernel_size, 1), 0))
            ),
        ])
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enc_p = TextEncoderWithQWFP(
            n_vocab,
            inter_channels,
            hidden_channels,
            filter_channels,
            ...
            q_teacher_forcing=q_teacher_forcing,
    ...
    def forward(self, x, x_lengths, y, y_lengths, features: BatchPadded | None = None, sid=None):
        ...
        with torch.no_grad():
            ...
            if self.use_noise_scaled_mas:
                epsilon = (
                    torch.std(neg_cent) * torch.randn_like(neg_cent) * self.current_mas_noise_scale
                )
    ...
    def infer(
        ...
        x,
        x_lengths,
        features: BatchPadded | None = None,
        sid=None,
        noise_scale=1,
        length_scale=1,
        noise_scale_w=1.0,
        ...
        if self.n_speakers > 0:
            ...
            g = None
        x, m_p, logs_p, x_mask, _ = self.enc_p(x, x_lengths, features=features, g=g)
        if self.use_sdp:
